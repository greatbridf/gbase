...
namespace gb
{
    basic_socket::basic_socket(int domain, int type, int protocol)
        ...
        _addr(sockaddr()), _timeout(DEFAULT_SOCKET_TIMEOUT)
    {
    ...
    basic_socket::basic_socket(int domain, int type, int protocol, int socket, sockaddr addr) :
        _domain(domain), _type(type), _protocol(protocol), _socket(socket), _addr(addr),
    ...
    socket::socket() : basic_socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)
    {}

    socket::socket(const char* ip, port_t port) : socket() {
    ...
    std::unique_ptr<client_socket> server_socket::accept() {
        if (_state != LISTENING) return nullptr;
        sockaddr_in c_addr = {};
        socklen_t len = sizeof(sockaddr_in);

        int socket = ::accept(_socket, (sockaddr*)&c_addr, &len);
        if (socket < 0) {
            if (errno == SOCKET_ERROR_CLOSED) close();
            return nullptr;
        }
    ...
    {
        if (_socket.state() == socket_states::CLOSED) return EOF;
